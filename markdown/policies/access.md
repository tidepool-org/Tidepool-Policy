...
standards. Tidepool uses `1Password` to securely store secrets internally, and uses `sops`
to save encrypted secrets to Github securely in plain text.
...
* A privileged user is someone who has full administrative access to critical
systems, such as the root user to a Linux/Unix system, and Administrator or
Root User to an AWS account.
